...
class TeamController extends Controller
{
    public function team(Request $request)
    {
        try {
            ...
            if($request->teamName){
                $data= Team::whereNotIn('id',$teamID)->where('title','like','%'.$request->teamName.'%')->skip($page)->take($size)->get();      
                return $this->success($data);
            }
            $data= Team::whereNotIn('id',$teamID)->skip($page)->take($size)->get();      
            return $this->success($data);
        } catch (\Throwable $th) {
    ...
    public function joinTeam(Request $request)
    {
        try {
            ...
            if ($state) {
                ...
                $state= JoinTeamNotice::where('team_id',intval($request->team_id))->where('user_id',$user->id)->get();
                if(count($state) != 0){
                    return $this->failed('已申请过此团队');
                }
    ...
    public function myTeam(Request $request)
    {
        try {
            ...
            $teamID= TeamMember::where('user_id',$user->id)->where('status',1)->pluck('team_id');
            $data= Team::whereIn('id',$teamID)->where('status',1)->skip($page)->take($size)->get(['id','cover','title']);
            return $this->success($data);
        } catch (\Throwable $th) {
            return $this->failed($th->getMessage());
        }
